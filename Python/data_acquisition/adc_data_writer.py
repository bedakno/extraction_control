...
class ADCDataWriter(DataWriter, ADCBoard):
    ...
    def __init__(self, outfile='output', comments = ''):
        ...
        """
        ADCBoard.__init__()
        DataWriter.__init__(outfile=''.join([outfile,'.csv']), columns=self.columns, comments=comments)
    ...
    def _get_values(self):
        """reads adc values matching the columns and adds a timestamp to them

        Returns:
            _type_: dictionary with column name and matching value
        """
        values = self.read_channels(self.columns[1:])
        timestamp = {self.columns[0]: time()-self.time0}
